...
function resolveBody(body: string): string {
  return body
    .replace(/\{\{firstName\}\}/gi, PLACEHOLDER_LEAD.firstName)
    .replace(/\{\{fullName\}\}/gi, PLACEHOLDER_LEAD.fullName)
    .replace(/\{\{company\}\}/gi, PLACEHOLDER_LEAD.company)
    .replace(/\{\{jobTitle\}\}/gi, PLACEHOLDER_LEAD.jobTitle)
    .replace(/\{\{operador\}\}/gi, 'Seu Nome (Operador)');
}
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={cn(
                ...
                showPreview
                  ? 'bg-gold-50 dark:bg-gold-900/30 border-gold-200 dark:border-gold-800/50 text-gold-600 dark:text-gold-400'
          ...
          {showPreview ? (
            <div className="px-8 py-6 space-y-4">
              ...
              <div className="text-xs text-slate-400 dark:text-slate-500 flex flex-wrap gap-2">
                ...
                {['{{firstName}}', '{{fullName}}', '{{company}}', '{{jobTitle}}', '{{operador}}'].map((v) => (
            ...
            <form action={formAction} className="px-8 py-6 space-y-5">
              ...
              <div>
                ...
                <input
                  ...
                  required
                  className="w-full bg-transparent dark:bg-slate-950 border border-slate-200 dark:border-slate-800 dark:text-slate-100 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold-500/30 focus:border-gold-500 transition-all placeholder:text-slate-400 dark:placeholder:text-slate-500"
                  ...
                  {Object.entries(TEMPLATE_CHANNEL_MAP).map(([value, { label }]) => (
                    <label key={value} className="flex-1 cursor-pointer">
                      ...
                      <div className={cn(
                        ...
                        channel === value
                          ? 'border-gold-500 dark:border-gold-500 bg-gold-50 dark:bg-gold-900/30 text-gold-600 dark:text-gold-400'
                          : 'border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:border-slate-300 dark:hover:border-slate-700'
                      )}>
                        {label}
                      </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <button
                    ...
                    className="flex items-center gap-1.5 text-xs font-bold text-gold-600 dark:text-gold-400 hover:text-gold-800 dark:hover:text-gold-300 disabled:opacity-50 transition-all px-2 py-1 rounded-lg hover:bg-gold-50 dark:hover:bg-gold-900/30"
                  >
                    {isImproving ? (
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="submit" disabled={isPending}
                  className="flex-1 px-4 py-3 bg-gold-600 hover:bg-gold-700 text-white font-semibold rounded-xl text-sm flex items-center justify-center gap-2 disabled:opacity-50 transition-all shadow-lg shadow-gold-200/50 dark:shadow-none">
